...
$(document).off('submit', '#check-delivery-form-edit').on('submit', '#check-delivery-form-edit', function (e) {
    e.preventDefault();
    let form = $(this);
    let submitButton = form.find('.edit-delivery');
    ...
    $.ajax({
        ...
        contentType: false,
        success: function(response) {
            submitButton.prop('disabled', false).html('Change Pincode');
            if (response.success) {
                $('#courier_partner').html(response.html);
            }
        },
        error: function(xhr) {
            submitButton.prop('disabled', false).html('Change Pincode');
            let errors = xhr.responseJSON?.errors;
            if (errors && errors.pincode) {
                $('#pin-message').text(errors.pincode[0]);
            } else {
                $('#pin-message').text('Something went wrong');
            }
        }
    });
});